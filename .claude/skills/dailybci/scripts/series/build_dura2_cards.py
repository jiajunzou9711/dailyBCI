...
# 08 脑移位
gen.figure_card(
    F("fig-shift.png"), "Hill 等 1998，21 名患者的术中位移测量",
    [
        "第三项与前两项性质不同。**硬膜位移均值 1.2 mm**；开硬膜后的脑表面，第一次测均值 **4.4 mm**，"
        "间隔约一小时的第二次测 **5.6 mm**。方向一致，全部是脑相对术前位置下沉。",
        "两个边界要一起报：该研究的**测量误差就是 1–2 mm**，1.2 mm 只能说落在分辨率附近；"
        "而**最大位移超过 10 mm** 的，第一次测量约占 1/3 患者、第二次约占 1/2。",
        "硬膜读数取自开硬膜前、脑表面取自开硬膜后。这个先后顺序支持一个推论："
        "**大位移主要由开硬膜后的脑脊液流失驱动**。目前只有这一项间接证据，标为推论。",
    ],
    P("08-shift.png"), figure_height=540,
)
