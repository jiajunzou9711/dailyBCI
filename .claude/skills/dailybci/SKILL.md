...
- **⛔ 每张内容卡的标题必须各不相同(2026-09-08 用户定,推翻此前的「逐字一致」写法)。** 连续几张卡顶着同一个标题,读者看着很不舒服,也看不出这一张到底讲什么。**每张卡的标题要归纳这一张自己的内容**,是这一张的一句话说明,不是所属章节的名字。
- **目录卡因此要写得简洁**——它列的是章节(5 个左右的大步骤),不是逐卡标题。目录条目与卡片标题**不再要求逐字一致**,只要读者能把卡片归到对应章节即可;圈号编号仍然共用同一套(同一章节下的多张卡沿用该章节的圈号,但标题各写各的)。
