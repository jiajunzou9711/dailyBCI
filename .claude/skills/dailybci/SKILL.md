...
- **封面图也可以直接用论文的实验设计图(2026-08-06 定,自制 SVG 之外的另一条路)。** 那一期两版并排做出来比过:自制概念图与标题同向但视觉偏素,论文原图可信度高、信息密度大,用户选了后者。**允许用,但要过三条:** ① **与标题同向**——标题说"没测到",就不能配一张只画"打中了"的图(当期第一版正是栽在这里,后来改用含对照条件的整块图 A 才解决);② **裁切完整**——不能切断解剖结构或截断标注,宁可多带一点周边内容;③ **去掉面板字母**(A/B 之类),那是论文排版的东西,不是给读者看的。
