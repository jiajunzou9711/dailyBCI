...
- **概念图是必须项,由 `cover_card(..., concept_image=...)` 传入**(2026-08-05 为此在 `card_generator.py` 加了 `concept_image` / `concept_height` 参数与 `.conceptwrap` 样式)。
- **概念图的硬约束:它是"概念",不是示意图。** 绝不能是流程图、结构图、带框线和箭头的机制图——那些属于正文图卡。它要满足三条:① **半秒内被看懂**;② **与标题互相加强**,而不是把标题的字面意思再画一遍;③ **尽可能简洁**,少文字甚至零文字标签(标签会把它从概念图拖向示意图,解释留给正文卡)。本期示例:标题「脑刺激实验最难的一步/证明变化确实来自刺激」,概念图画四条颜色不同的线汇进同一条脑电波形——一眼说明"读数里混了四样东西且长得一样",提出问题而不给答案。
- 概念图同样用自制 SVG(`scripts/series/<slug>_figs.py`)产出,渲染成 PNG 后传给 `cover_card`。
