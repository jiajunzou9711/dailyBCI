...
> - **必须做好引用**:该图出自哪一篇,在**目录卡的来源区**写明(封面本身不放溯源文字);若这张图与正文主引论文不是同一篇,尾卡参考文献里也要有对应条目,并与正文角标序号一致。
...
- 边距慷慨（100px+），内容不要贴边
- 封面卡只放：标题（放大）+ 一句话核心发现 + **一张论文真实原图的裁图**（2026-08-10 定，不再用自制概念图，见 Step 7 §封面）+ 品牌/日期（极淡）。**来源/溯源放目录卡,不放封面**（2026-08-05 定,见 Step 7）
- 详细数据、引用、分析留给后续卡片
