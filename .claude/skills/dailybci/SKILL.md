...
#### ⛔ Step 6 之后的三步顺序写死，不许打乱、不许跳步（2026-08-31 用户定）

事实核查表交付、✗ 与 ⚠ 全部清完之后，**立即按下面的顺序走完，不要再问"下一步做什么"，也不要先去裁图或先出标题**：

1. **封面标题讨论（紧接 Step 6，独立一轮）** —— 核查一完成就开这一轮。给 **3–5 个封面标题候选**并说明各自的取舍，等用户选定或改定。此时**只讨论标题**；封面配图方案可以同轮附上，但标题是这一轮必须闭掉的东西。判据仍是既有那条：遮住正文只念标题，读者要能说出"这篇讲的是什么东西、提了什么问题、给了什么答案"。
