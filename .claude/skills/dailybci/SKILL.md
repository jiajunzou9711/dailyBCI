...
#### 自制示意图的两条硬规则(2026-08-06 定)

来源:评估 `baoyu-article-illustrator` 这个第三方配图 skill 时,发现它的两条核心原则与本项目已有的硬规则同向,故写死。**该 skill 本身不安装**——它的出图后端是扩散模型(FLUX 一类),渲染不出准确的文字、数字、坐标轴与刻度,而本项目的示意图全是承重信息(`20.5 ms`、`1540 m/s`、疏密带的相对位置、箭头方向),错误的图比没有图更糟。**自制 SVG + Chromium 这条路线对技术示意图严格更优,不要替换。**

1. **画概念,不画比喻的字面物。** 图要呈现的是底层概念本身,不是把某个比喻画出来。这是"少用比喻"这条硬规则在图上的延伸(封面那条"概念图必须是概念而非流程图"是它的近亲)。反面例子:讲"电极听力半径"时画一只耳朵;讲"参数空间"时画旋钮。
2. **图上的标签必须用文中的真实数据与术语,禁止占位符。** 数值、单位、术语、坐标刻度一律用当期实际的数(`0.15 µm`、`487.5 Hz`、`一个波长 ≈ 3.1 mm`),不许写"约 X"、"数值待定"、"Value"。自算的数按 Content Standards 的要求在正文注明是自算。
...
### Step 2.5: 候选都不合适时,转 Mode C 原理期(2026-08-29 用户定)

**触发条件**:Step 2 呈现候选后,用户表示都不感兴趣;或当天扫完确无值得讲的新工作。**此时不要硬讲一篇**,也不要跳过当天。

做法:打开 `.claude/skills/dailybci/series-backlog.md`,从「未闭环」里挑 1–3 条报给用户,由他选一条,然后按 Mode C 走原理期。

两条产出线的分工:
- **A 类(日报)** —— 追当天的新工作,Mode A。
- **C 类(原理期)** —— 把某个工程/物理原理打穿,Mode C。**一期只闭一个点,当期必须闭环**;闭不了就如实标"待核"留到以后,**不许当期扩成连载**。

**不写死频次。** 哪天没有值得讲的新论文就走 C 类,有就走 A 类。用户的理由:C 类之间天然形成间隔,每次做新一期都要先回忆前一期,这个提取过程本身就是学习收益。

**每期出稿后维护 backlog**:把当期闭掉的点移到「已闭环」并注明日期与期号;当期没讲透的地方随手加进「未闭环」。日报(A 类)里挖出的原理问题同样往这里加——今天讲不完的,不是丢掉,是入列。
...
- **⛔ 某一块讲完、用户确认、且不再有悬而未决的追问时，必须主动提示落稿（2026-08-12 用户定，写死）。** 原话："一旦有某一个部分我已经确认了，且没有更多的问题了，你就要提示我是否要把它落稿。" 做法：**当场把那一块转成成品图卡文案**（三段式、圈号标题、承重点挂角标、去掉全部脚手架词），写进当期草稿文件，并告诉用户已落稿；然后再回到未完的讨论。**判据是"这一块有没有未决问题"，不是"整期讲完没有"**——讨论会长时间在某个细节上盘旋，已确认的内容若不当场固化，后面要重新回忆、极易走样或丢失。这是记忆 `dailybci-on-the-fly-drafting`（边讨论边定稿）的强化版：那条说"有价值的内容当场定成文字稿"，这条进一步要求**由我主动提示，不等用户想起来**。
- **⛔ 全程维护一份实时草稿 `output/<日期>-<slug>/draft.md`，会话里不再整篇复述（2026-08-12 用户定，写死）。** 用户原话：把讨论/出稿过程实时记进 draft.md，"我可以打开去看，然后这样也不会影响我们的正常的会话，或者说让它变得太长"。做法：
  - **文件是唯一的成稿载体**，讨论一有进展就 on-the-fly 写进去（新定稿的卡、改掉的措辞、新核实的数字、新产生的 TODO），**不要等整期结束再一次性写**。
  - **会话里只报增量**——"改了哪三处 / 新落了哪一卡 / 新增了哪条素材"，并给出文件链接，**绝不把整份草稿或整卡正文再贴一遍**。用户要看细节自己开文件。唯一例外是用户明确要求当场看某段。
  - **固定四节结构**：`已定稿`（卡号 + 圈号标题 + 正文 + 配图文件名 + 承重点/角标对应 + 诚实边界旁注）、`讨论中`（每卡记已确认要点与 `- [ ]` 未决项）、`备用素材`（已核实但尚未分配位置的数字与事实）、`参考文献`（按正文首次出现顺序，待最终编号）。
  - 最终由 draft.md 转成 `scripts/series/build_<slug>_cards.py`，与该期成品卡同目录。
- **⛔ 每次落稿后立即跑一遍措辞自检，不要等用户来抓（2026-08-12 用户定，写死）。** 起因：卡 9 落稿后由用户点出"花样"属比喻，我复查时才发现同一段还踩了写死的禁用句式「不是……而是」——**事后靠人眼抓，必然漏。** 落稿动作的最后一步固定执行：

  ```bash
  grep -nE "不是.{0,24}而是|并不是.{0,20}而是|不只是.{0,20}而是|而非|并非|花样|活不过|旋钮|拄|戒了|命运|欠账|还债|债务|买到了|付出.{0,4}代价|告诉我们|承认|堵掉|尺子|分不开|出路|压着|算数|链子|骨架|主脊|锋利|一针见血|石破天惊|颠覆|震撼|最硬|最有力|最关键的一句|追上|射程|\\\$.*\\\$|承上|转场|（开头）|（主体）|（结论）|Q[0-9]" output/<日期>-<slug>/draft.md
  ```

  命中即改，改完复扫到无命中为止。三类命中各自的处理：
  1. **禁用句式**（不是…而是）→ 改肯定陈述或"关键差别是 / 相比 X，Y 更…"。
...
#### ⛔ 禁用词「骨架」「主脊」与夸张类修辞(2026-08-27 用户定,写死)

**同时禁止夸张类的评价性修辞**——「最锋利的一句」「一针见血」「石破天惊」「颠覆」「震撼」这类。判断句的分量应由内容本身承担,不靠形容词加码。反例:「由此得到全文最锋利的一句推论」→ 改成「由此得到的推论是」。已并入落稿自检 grep。

**成品文字与对话里都不再用「骨架」「主脊」这两个词。** 它是把建筑/解剖的部件套到抽象结构上的比喻,与 [[avoid-metaphor-prefer-plain]] 同类。改成字面陈述:「这五条性质」「这套结构」「保留下来的部分」「贯穿全系列的那条判断」。已并入落稿自检 grep。

#### ⛔ 脑区名首次出现须附英文全称(2026-08-29 用户定,写死)

**图卡与文稿里出现脑区、核团、脑回等解剖结构名称时,首次出现必须在括号内附英文全称**——「梨状叶(piriform lobe)」「丘脑(thalamus)」「感觉运动皮层(sensorimotor cortex)」「丘脑底核(subthalamic nucleus)」。理由:中文神经解剖译名不统一(尤其非人物种的脑回,如犬的 ectomarginal / suprasylvian gyrus),只给中文读者无法回查文献。译名确实没有通行中文的,直接保留原文名称并说明。落稿自检时逐条扫脑区名。

#### ⛔ 以某篇论文为主线时,照论文自己的叙述逻辑走(2026-08-29 用户定,写死)

**一期若以某篇论文为主线,默认遵照该论文自己的叙述逻辑、术语与划分**;只有当论文确实没讲清、或确有引申必要时才另起框架,且必须**标明哪部分是论文的、哪部分是我加的**。
起因:CorTec BIC 那期我把闭环延迟讲成「采集/检测/下令/刺激」四段,论文实际只分**两段**(采集延迟、刺激延迟)。自制框架读起来顺,但会让读者以为那是论文的划分,属于事实失真。用户判据:**解读一篇论文,主线就该是这篇论文。**
操作:讲解与出稿前先问「这个划分是论文的还是我的」。是我的就要么改回论文的,要么就地标注来源。与 [[dailybci-paper-as-hook-science-line]](论文作引子、正文讲科学线)是两条路线,后者仅当期明确要那样写时才用。

#### ⛔ 直接给答案,不用设问式卖关子(2026-08-27 用户定,写死)

**成品文字里默认用陈述句直接给出答案,不用「对到什么程度——」「那么问题来了」「这意味着什么?」这类设问式过渡。** 设问把已经知道的答案推迟一句给出,读者付出的是注意力,换回的信息为零。
- 反例:「对到什么程度——相关达 0.974」→ 改成「一个具体的量:相关达 0.974」。
- **图卡标题必须是断言式的结论,不能是过渡句。** 标题要提纲挈领,读者只看标题也能知道这张卡的判断是什么。反例:「相似发生在哪一层」「但两者显然不是一回事」→ 改成「模型与人脑的相似已被反复测到」「作者自己列出了十几条差距」。
- 例外:真正要交给读者去想、且下一张卡才回答的分析性问题可以保留,但一期最多一处。
- 与 `⛔ 少用否定式断言` 同向:那一条管语气的正负,这一条管**该不该直接给答案**。
...
**出分卡文字稿之前,先明确本期属于四种张力类型的哪一种,并写进 `draft.md` 抬头。** 本条只适用于 DailyBCI 的内容生产,不适用于本仓库的其他项目。

| 类型 | 读者的起点 | 结构 |
|---|---|---|
| ① **未知型** | 不知道某件事 | 先把"你不知道"建立起来 → 快速给答案 → 展开 |
| ② **纠错型** | 以为对的事其实站不住 | 复原旧认知 → 指出它依赖的证据基础 → 引入反例 → 给新结论 → 划定强度边界 |
| ③ **化繁为简型** | 以为很复杂 | 摆出表面的复杂 → 给出那条简单主线 → 用主线重解各处细节 |
| ④ **由简入繁型** | 以为很简单 | 摆出那个"显然"的说法 → 拆开第一层 → 露出整套机制 |

**钩子与落点同源:开篇的钩子和末卡的落点必须是同一个类型的两端。** 选型定了,大逻辑的结构就定了,不必每期重新摸索结构。

**落点句必须与所选类型同向**——纠错型的落点是"旧结论错在哪、错到什么程度",化繁为简型的落点是"那条简单主线是什么"。**强度要收准**:纠错型尤其容易写过头,"削弱支持它的证据"与"直接证伪"是两回事,写成后者就是失真。

#### ⛔ 文献解读期的标准七段结构(2026-09-01 用户定,写死)

**凡是以某一篇论文为主线的解读期(Mode A 日报的主流形态),默认按下面七段排提纲。** 细节每期可调,整体顺序不再每期重新摸索。用户 2026-09-01 定:"这个整体架构可以确定下来了。"

| 段 | 承担什么 | 典型卡数 |
|---|---|---|
| **一 · 旧认知与它的裂缝** | 复原读者(和领域)现在持有的那个说法,以及它依赖的证据;然后给出削弱它的观测 | 3–5 |
| **二 · 遗留的问题** | 裂缝只说明"不是 A",没说"那是什么"。把问题收敛成一个可回答的形式 | 1 |
| **三 · 作者怎么问、打算怎么答** | 论文的**原问题**(照原文措辞)+ 作者自己划的边界 + 解法思路;**答案在本段末句一句话先给** | 1–2 |
| **四 · 方法** | 实验设计、被试/物种/电极模态、关键量怎么测 | 2–3 |
| **五 · 结果** | **按论文自己的结果小节顺序走**,一节一卡 | 4–5 |
| **六 · 闭环** | 回到第一段的裂缝,逐条说明新答案怎么解释它们 | 1–2 |
| **七 · 引申与强度边界** | 回到领域的大问题:这个结论能支持什么、**不能**支持什么。纠错型尤其要在这里收准强度 | 1–2 |

三条配套硬规则:
1. **答案不单独成卡**,并进第三段末句。单卡只放一句结论既浪费版面,又会与第六段的闭环重复。
2. **第六、七段是落点,不许为省版面压缩。** 纠错型最常见的失真是把"削弱了支持某结论的证据"写成"证伪了该结论"。
3. **第五段照论文的划分走**(见上方 ⛔ 以某篇论文为主线时的规则);自加的框架必须就地标明是我加的。

与「张力类型先行」的关系:张力类型定**钩子与落点的性质**,本结构定**段落顺序**。两者同时用:选定类型 → 按七段填。

#### 图纲先行(2026-08-06 定,与分卡文字稿同时出)

**出分卡文字稿的同时,出一张「图纲表」,把每张图先定下来再动手画。** 起因:TUS 那期直接开写 `*_figs.py`、边写边想这张图画什么,结果第一版三处返工(说明文字溢出、疏密标签与实际疏密带错位半个波长、两个小图的密/疏差别看不出来)。**先写清"这张图要让读者看懂哪一件事",再写代码,返工少得多。** 这与"出提纲阶段就先数卡数"是同一层动作,只是把图也纳进来。
...
| 类型 | 用于 | 本项目实例 |
|---|---|---|
| **framework** | 模型、结构、层次关系 | 三层控制变量的递进 |
| **comparison** | 并列对照 | 三篇 LGN 工作的物种/颅骨/强度/结果对照 |
| **flowchart** | 过程、因果链 | 压缩区稀疏区的产生与传播 |
| **timeline** | 历史演进 | TUS 从 1958 到今天 |
| **infographic** | 数据、指标、量的关系 | 两个机械成分随脉冲时长的分化 |
| **toc** | 本期路线 | 目录卡(每期强制,见 Step 7) |

**`scene`(叙事/情绪向)本项目不用**——与"学术简洁、不煽情"的设计规则冲突。

图纲定完再写 `scripts/series/<slug>_figs.py`。**渲染出来仍要逐张 `Read` 自检**(溢出、错位、构件齐不齐),自检通过才 `SendUserFile` 推给用户。
...
#### ⛔ Step 6 之后的三步顺序写死，不许打乱、不许跳步（2026-08-31 用户定；2026-09-02 合并第 1 步）

事实核查表交付、✗ 与 ⚠ 全部清完之后，**立即按下面的顺序走完，不要再问"下一步做什么"，也不要先去裁图**：

1. **核查 + 封面标题提案合成一轮交付（2026-09-02 用户定，覆盖此前"独立一轮"的写法）** —— **同一条消息里**先给三层核查表、说明 ✗ 与 ⚠ 各自怎么改掉的，**紧接着直接给 3–5 个封面标题候选**并说明各自的取舍。不要交完核查表就停下来问"要不要开始出标题"。用户的理由：核查通过之后出标题是确定要做的下一件事，中间那次征询是空转。此时仍**只讨论标题**；封面配图方案可以同轮附上，但标题是这一轮必须闭掉的东西。判据仍是既有那条：遮住正文只念标题，读者要能说出"这篇讲的是什么东西、提了什么问题、给了什么答案"。
   - **前提没变**：核查必须真的跑完、✗ 与 ⚠ 必须真的清完，标题才跟在同一条消息里。**承重项还挂着 ⚠ 时不许出标题**，那种情况按 Step 6 的分流表上交用户拍板。
2. **封面标题定稿后，直接出最终渲染版图卡** —— 裁图、目录卡、正文卡、尾卡、封面一次性渲染到位，`SendUserFile` 全套推给用户。不再中途征询单张卡（封面已在第 1 步定完，正文卡在 Step 4a/5 已逐张落稿）。
3. **与图卡同一次交付，一并给出发布元数据三样** —— 小红书发布标题推荐、话题标签推荐、微信公众平台摘要（≤120 字字段版 + 200–300 字导语版）。做法照 §发布标题 + 话题标签 与 §最终交付固定是三件套 两节，全部放进代码块供直接复制。

三步走完即为「最终稿交付」，然后**停下等用户确认**，确认后才进 Step 9（见 Step 7 顶部与 Step 9 的进入条件）。

**为什么把封面标题提到渲染之前**：封面标题是整期定调的东西，它一改，目录卡的措辞、正文卡的落点句、乃至发布标题都可能要跟着动；等图渲染完再讨论标题，前面那一轮渲染就白做。（这与既有的「封面单独走一轮打磨、不与正文卡一起批量出」是同一条规则，此处把它的**时间点**也钉死在 Step 6 之后。）
...
**⛔ 封面单独走一轮打磨,不与正文卡一起批量出(2026-08-12 用户定,写死)。** 用户原话:"封面非常重要……其他这个后面的文稿和渲染可以一次性生成,但是封面我们可能会要多纠结一下,多较一下真。" 流程:
1. 正文卡(含目录卡、尾卡)按定稿**一次性生成并渲染**,不逐张征询。
2. **封面单独提案、单独确认**:先把**标题候选 + 配图方案**列出来与用户讨论,**确认后才渲染封面**。别在批量渲染时顺手把封面一起做掉。
3. 封面标题的判据(2026-08-12 因一版被打回而补):**遮住正文只念标题,读者要能说出"这篇讲的是什么东西、提了什么问题、给了什么答案"。** 被点名的反例:「视皮层通什么电／让模型反推出来」——表达诡异,读者不知道在说什么、解决了什么、答案是什么。
4. **封面配图二选一,不要中间态**:① 确实需要多个 panel 才说得清 → 用完整的多 panel 图;② 否则 → **裁单个 panel,放到最大、最清楚**(less is more)。被点名的反例:拼了两栏但每栏都不大,两头不靠。
5. **⛔ 裁图的源分辨率决定它在卡上能有多大——`card_generator` 不会把图放大到超过原生像素(2026-08-12 实测踩坑)。** 那次封面图"怎么调 `concept_height` 都不够大",根因是裁图源自 **300 dpi** 的页面渲染,拼出来只有 576px 宽,而卡的内容宽是 **880px**,于是渲染时按原生 576px 显示、两侧留白。**规则:凡是要铺满卡宽的图(封面尤其),必须从 `get_pixmap(dpi=600)` 及以上的页面渲染里裁**,保证裁出的成品**宽度 ≥ 1000px**;坐标可直接按 dpi 比例从 300 dpi 的量测结果换算(600 dpi 即 ×2)。正文图卡同理:图偏小时先查源图宽度,再去调 `figure_height`。

**⛔ 默认板式(2026-08-05 用户定,写死,以后每期照此出,不必再问):**

**封面 = 标题(两行) + 一句话核心句 + 一张论文真实原图。三样,没有别的。**

> **⛔⛔ 封面图必须是论文里真实截出来的图,不再用自制概念图(2026-08-10 用户定,写死,从这天起每期照办、不必再问)。**
> - **做法**:从当期引用的论文 PDF 里裁出一张图(裁法见 Step 8 的投影定包围盒 + 逐边自检),经 `cover_card(..., concept_image=<裁出的PNG>)` 传入——参数名沿用 `concept_image`,但喂进去的内容改成论文原图。
> - **必须做好引用**:该图出自哪一篇,在**目录卡的来源区**写明(封面只放本节下面那行极简「解读来源」小字,完整溯源仍在目录卡);若这张图与正文主引论文不是同一篇,尾卡参考文献里也要有对应条目,并与正文角标序号一致。
> - **仍要过 2026-08-06 定的三条**:① **与标题同向**(标题说"没测到",不能配一张只画"打中了"的图);② **裁切完整**(不切断解剖结构、不截断标注,宁可多带一点周边);③ **去掉面板字母**(A/B 之类是论文排版,不是给读者看的)。
> - **背景介绍期(不绑单篇论文)同样适用**:从当期实际引用的任一篇里选一张最能代表本期主线的图,按上面同一套规则处理并注明出处。
> - **自制 SVG 概念图不再用于封面**;自制 SVG 仍用于正文示意图与目录卡(见 Step 5「图纲先行」与本节目录卡)。下面关于概念图的几条,**保留为历史记录,不再执行**。
- **⛔ 封面必须带一行极简的「解读来源」小字(2026-08-31 用户定,覆盖此前"溯源一律不放封面"的写法)。** 内容固定三项、一行说完:**何时发布 · 论文标题 · 本期是对它的解读**,例如「2026 年 8 月 bioRxiv《A wireless modular platform for neuro-behavioral recording and closed-loop manipulation in small animals》· 本期为该文解读」。要求:**字号极小、灰度极淡**(走 `source` 位),读者一眼扫过不干扰标题,但想确认"这是在解读哪篇"时立刻找得到。
- **完整溯源仍放目录卡**——团队/机构、通讯作者、为什么权威、方法一句话,这些**不进封面**,封面只留上面那三项。理由:封面要在读者滑动的半秒里换来一次点击,长溯源帮不上忙;但"这是对某篇论文的解读"属于身份声明,不属于溯源细节,必须在封面就说清,免得读者误以为是原创研究或泛泛科普。
...
- **封面图位是必须项,由 `cover_card(..., concept_image=..., concept_height=...)` 传入**(2026-08-05 为此在 `card_generator.py` 加了这两个参数与 `.conceptwrap` 样式)。**2026-08-10 起喂进去的是论文原图裁图,不是自制概念图**——参数名未改,内容含义已改。

**↓↓ 以下三条为 2026-08-05/06 的概念图规则,已于 2026-08-10 停用,仅作历史记录保留,不要照做 ↓↓**
...
- ~~概念图同样用自制 SVG(`scripts/series/<slug>_figs.py`)产出~~。**注**:自制 SVG 的背景色须设成与卡面一致的 `#FAFAFA`(而非默认白),否则卡上会出现突兀的白色方块——这一条对**目录卡与正文示意图仍然有效**。

**↑↑ 历史记录到此为止 ↑↑**

- **封面用论文原图时要过的三条(2026-08-06 定,2026-08-10 升为唯一路线,仍然有效):** ① **与标题同向**——标题说"没测到",就不能配一张只画"打中了"的图(当期第一版正是栽在这里,后来改用含对照条件的整块图 A 才解决);② **裁切完整**——不能切断解剖结构或截断标注,宁可多带一点周边内容;③ **去掉面板字母**(A/B 之类),那是论文排版的东西,不是给读者看的。
- **`cover_card(..., title_top=...)` 可收紧标题上方留白,把版面让给图(2026-08-06 加)。** 模板默认 `.title { margin-top: 200px }`,封面上方常有约 200px 死白;传 `title_top=95` 之类可把它收掉,图随之能从"宽 756px"长到"铺满内容区 880px"(面积 +35%)。**默认值不变(200),不传即与以往各期一致。** 调完必须重新跑一次溢出自检,确认图底不压页脚。

**⛔⛔ 每一张内容卡都必须有标题,且与目录卡逐条对应(2026-08-10 用户定,写死,以后每期照办)。**
- **做法**:`figure_card(..., title="…", title_num="①")` 把标题渲染在**图的上方**(2026-08-10 为此在 `card_generator.py` 加了 `title` / `title_num` 参数与 `.cardtitle` 样式);纯文字卡用 `text_card(..., heading_lines=["① 标题", "副标题"])` 对齐同一套编号。
- **措辞必须与目录卡逐字一致**——读者在目录里看到的那一条,要能在正文卡上原样找到。这是导航,不是标题创作,别在正文卡上另起一套说法。
- **编号用圈号(①②③…)**,目录卡与正文卡共用同一套序号。
- **卡数多于目录条目时,允许多张卡共用同一个标题与编号**(同一条目录项拆成两三张讲),不必为了凑数硬拆目录。反过来,**目录条目数不足以覆盖全部内容卡时要补目录条**——出稿前对一遍:每张内容卡的标题都能在目录里找到吗?落点那张尤其容易漏。
- **标题占版面(约 1.7 行)**,渲染完必然要压缩注文。**按 less is more 直接砍文字,不要靠缩图或改字号硬塞**。砍的时候优先砍"复述图上已有内容"的句子——2026-08-10 那期卡 7 就是这么改好的:原文把图里的 32%/52%/57% 又抄了一遍,改成「下方那三个数是证据:单纯观察组反而最高」,既省地方又更符合图文对版的要求。

**下期预告(可选,不作硬规则;2026-08-10 用户定):** 多期连载时,可在收尾卡加一节「下一期 · <主题>」并列出下期要回答的 2–3 个**具体问题**(不写"敬请期待")。**但日报常常当天讲完就没有后续、也无从预知第二天出什么,所以这不是每期必须项。** 遇到成系列的选题时,**主动提醒用户「这一期可以带下期预告,要加吗」**,由用户决定。
...
**⛔⛔ 图卡注文默认与纯文字卡同字号,不许用 `annot_size` 缩字号硬塞(2026-08-17 用户定,写死)。** 起因:当期六张图卡我全传了 `annot_size=27`,把注文缩小塞进版面,被用户点名。**缩字号是在用排版手段掩盖「文字写太多」这个内容问题**;一套卡里图卡字比文字卡字小会显得割裂,而且缩字号会让文字量的上限不断放宽,与 less is more 直接冲突。四条:
1. **默认不传 `annot_size`**,走 CSS 默认(`SIZE_ANNOTATION`)。
2. 注文排不下时,**第一动作是砍文字,不是缩字号**。
3. **优先砍「图上已经能看懂的细节」**——图能负责的交给图。指认图里的元素是必要的(图文对版,见下方硬规则 3);复述图里的数值、坐标、颜色分布是多余的。
4. **确实必须保留这么多字、砍无可砍时,停下来把这一张交用户审核**,由他决定缩字号还是拆成两张卡。**不要自己默默缩。**
5. **唯一既有例外:目录卡的来源区**(2026-08-13 定,见上方§封面 + 目录卡)——溯源信息不许为腾地方而删,那里允许 `annot_size=27`。
...
5. **每张卡必须相对独立可读——逻辑承接 ≠ 理解依赖（2026-08-12 用户定，写死）。** 规则 4 的三段式要求卡与卡在**逻辑上**咬成一条链（承上、转出），但**读者的理解不能依赖回看上一张**。用户原话："不要让大家需要记住前一张卡才能看懂这张卡的内容，还是要相对独立一点吧。" 具体：① **共用视觉语言时每张图自带图例**（如相邻两卡都用"蓝＝正在被更新／灰＋锁＝冻结"，两张图都要各自标出）；② **需要与前一张作对照的差别，在本卡内把话说全**，不写"对照上一张图可见…"；③ 承上句是**接住上一张的结论**，不是**引用上一张的图或记号**。判据：把这张卡单独拿给没看过前一张的人，图和正文是否仍然自洽。
...
- **裁子图先用"留白投影"定位 panel 边界,不要肉眼猜像素坐标。** 反复目测 `crop()` 坐标既慢又总切不准(切掉时间序列右端、漏掉图例、把高出量程的误差棒截顶)。正确做法是让图自己告诉你边界:把整图转灰度→`ink = (pixels < 235)`→对**列**求和找出"基本无墨的列区间"(=左右 panel 之间的竖直留白沟),对**行**求和找出横向留白沟;panel 的真实包围盒就在相邻留白沟之间。多 panel 共用一行/一列时(如左列多子图紧贴、右列上方还压着别的 panel),要**只在目标列区间内**再投影一次行、或只在目标行区间内再投影一次列,才能把目标 panel 从邻居里分离干净。关键教训:**panel 的数据/坐标轴常比你以为的延伸得更远**(本期 Fig 2A 的时间序列一直画到 x≈1028,肉眼以为到 805 就完了,结果右端 1/4 被切),所以一定要用投影量出真实右/下边界,再加一点点统一边距。
  - **⛔ 投影的搜索窗若被人为限制,量出来的就是搜索窗的边界,不是内容的边界(2026-08-06 因此把封面上的脑切掉一条而写死)。** 当期为了避开右侧的黄色换能器标注,把搜索窗限制成 `g[149:434, 31:238]` 再求包围盒,得到右边界 237——而脑组织实际延伸到 **284**,封面上的脑因此被切掉约 48 像素,用户一眼看出"这不是一个完整的脑子"。**规则:量包围盒时搜索窗必须覆盖整个候选区域;要排除某个标注,用颜色或形态掩码把它剔除(如 `yellow = (r>170)&(g>130)&(b<100)`,再取 `content = bright & ~yellow`),而不是缩小搜索窗。** 量完还要核一遍:**量出的边界如果正好等于搜索窗的边界,就说明被截断了**,必须放大窗口重量。
- **⛔⛔ 单个 panel 必须裁干净——切掉本属于该 panel 的边角内容,等同不合格(2026-08-31 用户点名,写死)。** 起因:当期卡 ⑩(Fig 6h)、⑪(Fig 6i)被判定裁切不干净,边角上本属于该图的内容被切掉。**根因不是规则缺失,是我违反了已有规则**——为躲开相邻 panel 的残留(6g 拖下来的竖线、面板字母),我**手动把边界往里收**(6h 的 y 起点 620→631,6i 的 x 收到 617、y 收到 414),这正是本节上面那条"搜索窗必须覆盖整个候选区域,要排除某个标注就用掩码剔除、不许缩小搜索窗"所禁止的做法。
  - **硬规则:任何时候都不许为了躲干扰而手动内收边界。** 邻 panel 的残留、面板字母、别的图的拖尾——一律用**掩码**(颜色/形态/连通域)剔除后再量包围盒,量完再加统一外边距。
  - **自检必须能发现"框外还有内容"。** 当前做法(只 `Read` 裁出来的那张图逐边看)拦不住这类错误:人眼看一张孤立的裁图,看不出本该还有东西在框外。**对照检查是必须的**——把裁框画回原图上看,或量一遍"紧贴裁框外侧一圈有没有墨";量出的边界若正好等于人为设定的搜索窗边界,即判定被截断,必须放大窗口重量。
  - **⚠️ 更强的自检规则与裁图方法待重新设计**(2026-08-31 用户提出"以后要修改一下裁剪的方式方法以及最后能够通过的规则",约定下次讨论)。在那之前,至少执行上面两条。
...
专题与日报(Mode A)是**两条不同的 pipeline**:日报绑当天单篇论文、走 10 步关卡;专题不绑单篇、可跨多期、原理驱动。成品进 `output/series-<slug>/`,出图复用 `card_generator.py` + 自制 SVG 示意图(见记忆 `dailybci-series-deep-dive-track`)。已出:犹他阵列、电极材料、神经解码方法论、侵入式定位精度、**EEG 工频/阻抗均衡(series-eeg-impedance-01)**、**EEG 电极界面/基线漂移(series-eeg-impedance-02)**、**接地的第一性原理(series-grounding-01,上篇,2026-09-03)**。
...
- 边距慷慨（100px+），内容不要贴边
- 封面卡只放：标题（放大）+ 一句话核心发现 + **一张论文真实原图的裁图**（2026-08-10 定，不再用自制概念图，见 Step 7 §封面）+ **一行极简「解读来源」小字**（何时 · 论文标题 · 本期为该文解读；2026-08-31 定，见 Step 7 §封面）+ 品牌/日期（极淡）。**完整溯源（团队/通讯作者/方法）仍放目录卡,不进封面**
- 详细数据、引用、分析留给后续卡片
