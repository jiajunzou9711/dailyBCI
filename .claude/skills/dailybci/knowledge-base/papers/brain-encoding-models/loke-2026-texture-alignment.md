...
## 下篇解读补记（2026-09-02）
- **天花板的读法是这篇最容易被忽略的一处**：200 ms 后三条对齐曲线都回落，但纹理条件已贴近噪声天花板下界（可解释的解释完了），而原图与只留物体条件天花板更高、缺口更大——即晚期物体相关信号是**真实且被试间一致**的，只是现有前馈架构没抓住（Fig 3B）。只看曲线高低会把三个条件误读成一回事。
- **作者对「人类形状偏好」这一反问的回应**：形状偏好是**整条加工链的行为读出**（引 [[hermann-2020-origins-of-texture-bias]]），而现有 DNN 擅长的是 100–200 ms 的前馈前端；前端由纹理统计主导与末端表现形状偏好可同时成立。
- **未训练网络为何也有分**：若对齐反映的是对纹理类统计的共同敏感性，则架构精巧程度与训练**两者都非必需条件**。注意与 [[conwell-2024-inductive-biases-brain-predictivity]] 并读——训练仍是该篇最大最稳的效应（N=64 随机初始化对照），两条不矛盾。
