...
- [Eckstein 2024](papers/ai-neural-modeling/eckstein-2024-em-neurotransmitter-classification.md) — 3D 卷积网络从电镜突触图像块预测六类递质：单突触 87%、神经元 94%（FAFB）；给全脑连接组补上递质层。**预测的是递质不是正负号**，兴奋/抑制由受体决定
- [Lappalainen 2024](papers/ai-neural-modeling/lappalainen-2024-connectome-constrained-visual-network.md) — 果蝇视叶运动通路 64 个细胞类型的连接组 + 任务优化反推 734 个参数（每类型时间常数/静息电位、每对类型单位突触强度），预测与 26 项研究实测相符；与 Shiu 2024 只有 1 个自由参数形成对照
...
### 「对齐分数到底测什么」这一格 (2026-09-01 建)
本线 A 分支的**自我检验**：不问哪个模型更像脑，而问「脑预测力/对齐分数」这个指标本身能支持什么结论。评价任何"某模型更像脑"的说法前先过这三条。
