...
- **封面默认板式 = 标题(两行) + 一句话核心句 + 一张概念图,三样,没有别的**(2026-08-05 定):**来源/溯源信息移到目录卡**,不再放封面(封面换点击、目录做信任判断+导航)。概念图必须是**概念**而非流程图/结构图——半秒看懂、与标题互相加强、少文字甚至零标签;用自制 SVG 出图,经 `cover_card(..., concept_image=..., title_size=...)` 传入。**标题字号可用 `title_size` 调大到标题块约占封面 1/4**(内容宽 880px:120px 字号下每行最多 7 字);**`title_top` 可收紧标题上方的默认 200px 留白,把版面让给图**(2026-08-06 加,默认不变)。**封面图也可用论文的实验设计图**(2026-08-06 定),条件是与标题同向、裁切完整、去掉面板字母。完整规则见 SKILL.md Step 7 §封面 + 目录卡。
