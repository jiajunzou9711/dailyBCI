...
- **「专题」深度长文(常青,独立于日报)** → 不走 Mode A 的关卡;轻量流程 = 提纲→确认→双语文案→出图,piece-by-piece。成品进 `output/series-<slug>/`,出图复用 `card_generator.py` + 自制 SVG 示意图。已出:犹他阵列 lineage、电极绝缘材料(Parylene-C vs Polyimide)、神经解码方法论系列(四轴 / 生成式脊柱 / 三层嵌套)、侵入式 BCI 定位精度(上下两篇,各 17 卡)。体量大的专题拆成上/下多篇,每篇独立守 18 卡上限。细节见记忆 `dailybci-series-deep-dive-track`。
