...
- **图卡注文默认与纯文字卡同字号,不许用 `annot_size` 缩字号硬塞**(2026-08-17 定)。缩字号是在用排版手段掩盖「文字写太多」这个内容问题。排不下时**第一动作是砍文字**,优先砍「图上已经能看懂的细节」;确实砍无可砍才**停下来把那一张交用户审核**,由他决定缩字号还是拆卡。**唯一例外是目录卡的来源区**(溯源不许为腾地方而删,允许 `annot_size=27`)。完整规则见 SKILL.md Step 7。
...
- **⛔ 关于真人/课题组的方向性断言,一律回权威来源核实,不许凭印象写**(2026-08-13 定):"长期做 X""专攻 Y""是 Z 的开创者""某某的一作/通讯"——只要涉及真实人物或实验室,先核后写。核到本人/实验室的官方 profile、该论文自己的参考文献(自引最能坐实"这个组做过什么")、或 PubMed/Scholar;**搜索摘要的转述不算核实的终点**。核不到就只写能核到的部分(职称、单位、这篇的方法),或如实标"方向待核"。**优先用可回溯的具体事实替代模糊资历形容**——"同组 2022 年在 PNAS 上做过小鼠视网膜神经节细胞的活体单细胞钙成像"胜过"长期做视网膜成像"。用户判据:**这不是知识本身、涉及真人,所以必须准确**。目录卡「来源区」的"为什么权威"一句是最易编的位置;来源区变长时用 `figure_card(..., annot_size=27)` 收小字号,别靠删溯源腾地方。完整规则见 SKILL.md Content Standards → Factual Accuracy 第 5 条。
...
- **「数据看板 / 数据复盘」= 单一常驻交互看板 `dashboard/index.html`(2026-08-13 定)+ 抓取技能 platform-data-dashboard** → 双击打开 `dashboard/index.html` 即可用(无需本地服务器):顶部账户条(小红书粉丝/公众号总用户/更新时间)、时间筛选(今天/最近3天/最近7天/自定义/全部)、平台切换(小红书/公众号/双平台对比)、概览卡片、文章列表点开看单篇详情(完整指标 + 笔记诊断百分位 + 增长曲线)、底部「复盘·下一步」区。**数据持久化在 `dashboard/data.js`**(严格 JSON,同时是合法 JS,浏览器经 `<script src>` 读、file:// 无 CORS 问题),每次采集后**追加累积**而非覆盖——这是看板能画「单篇增长曲线」、能按「发布后第几天」公平对比的前提。**采集与合并**:复用 `~/.codex/skills/platform-data-dashboard/` 抓取流程(浏览器需已登录两平台,未登录先让用户手动登录;字段口径见其 references/wechat.md + xiaohongshu.md),抓完把当次结果写成 JSON 片段,再跑 `python3 dashboard/merge_data.py --input <抓取.json> --output dashboard/data.js` 幂等合并(同 slug 按日期去重覆盖)。**口径纪律**:缺失/未报告一律 null 不填 0;百分比存「百分数数值」。历史仅以 2026-08-06 快照为锚点,过去逐日增量不可恢复,从今起每天累积。
