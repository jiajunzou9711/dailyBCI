...
- **封面默认板式 = 标题(两行) + 一句话核心句 + 一张论文真实原图,三样,没有别的**(2026-08-05 定板式,2026-08-10 改配图):**来源/溯源信息移到目录卡**,不再放封面(封面换点击、目录做信任判断+导航)。**封面图必须是从当期引用论文里真实截出来的图,自制 SVG 概念图已停用**(2026-08-10 定;背景介绍期同样适用,从当期引用的任一篇里选)。裁图三条:与标题同向、裁切完整、去掉面板字母。经 `cover_card(..., concept_image=..., title_size=...)` 传入(参数名沿用,内容含义已改)。**标题字号可用 `title_size` 调大到标题块约占封面 1/4**(内容宽 880px:120px 字号下每行最多 7 字);**`title_top` 可收紧标题上方的默认 200px 留白,把版面让给图**(2026-08-06 加,默认不变)。**封面单独走一轮:正文卡按定稿一次性批量渲染,封面先提案标题+配图、确认后再渲染**(2026-08-12 定)。标题结构=**上行讲挑战、下行讲解法,两行都是肯定陈述**,禁疑问句;判据是遮住正文只念标题,读者能说出「讲什么、问什么、答什么」。配图**二选一不要中间态**:要么用确实需要多 panel 的完整图,要么裁单个 panel 放到最大。**裁图源必须从 600 dpi 以上的页面渲染取(成品宽 ≥1000px)**——`card_generator` 不把图放大到超过原生像素,源图太小时无论怎么调 `concept_height` 都铺不满卡宽。字号-字数:96px 每行 9 字、112px 每行 7 字、88px 每行 10 字。完整规则见 SKILL.md Step 7 §封面 + 目录卡。
- **每张内容卡都要有标题,且与目录卡逐条对应**(2026-08-10 定):`figure_card(..., title=…, title_num="①")` 把标题排在图上方,纯文字卡用 `heading_lines` 对齐同一套圈号;措辞与目录逐字一致(导航用,别另起说法)。**多张卡可共用同一条目录项的标题**;目录条数不足以覆盖全部内容卡时要补目录条。标题占约 1.7 行,渲染后按 less is more 直接砍注文,优先砍"复述图上已有内容"的句子。**下期预告为可选项**,成系列时主动提醒用户要不要加,不作硬规则。
...
- **Mode A 9 步**(末步 Step 9 收尾:知识库入库→清理 papers/ scratch→neat-freak→commit),三个强制确认关卡:**Step 2 选题**(表格候选 + 知识库对照 + 推荐;无对照基准弹缺位提示问是否补库)、**Step 4 insight**(先给带理由的主张 + **直接贴论文原图**,用户确认才走)、**Step 6 事实核查**(草稿后自动三层核查出表,⚠/✗ 按"承重×严重度"分流,全 ✓ 才进生产)。
...
- **Step 1 搜索面 = 预印本 + 正刊两条腿**(2026-08-04 定):除 bioRxiv/medRxiv/arXiv 外,每天必须单独扫一遍 Nature / Science / Cell / Neuron / Nat Neurosci / Nat Med / Nat BME / Curr Biol / Nat Commun / Sci Adv / NEJM 的最新上线页(刊单见 SKILL.md Step 1)。另两条同日写死:**候选不许只靠标题关键词过滤**(按类目全量看标题、可疑读摘要)、**报新鲜度必须核版本号**(区分"这一版的日期"与"首次公开的日期")。**2026-08-05 补第三条(因实际漏扫 Cell 主刊而写死):上表 11 个刊必须逐个扫完、逐个报——返回被截断要分片取全,禁止用出版节奏推断"本周没有新的",汇报用逐刊清单写明"扫到的最新一条日期"。**
...
- **「数据看板 / 数据复盘」= platform-data-dashboard 技能(2026-08-06 定)** → 拉小红书创作服务平台 + 微信公众平台后台数据,产出 `output/*-dashboard.html` 极简看板(单篇/近1-3篇、过去一周两模式,核心指标每平台 3 个),并给具体行动建议。主流程与抓取经验在 Codex 技能 `~/.codex/skills/platform-data-dashboard/`(SKILL.md + references/wechat.md + references/xiaohongshu.md + assets/dashboard-template.html),Claude 侧直接复用该技能文件。开跑前浏览器必须已打开并登录两个平台,未登录先让用户手动登录;数据口径见技能 references(平台显示 0/暂无数据时标「未报告」,不填 0)。
