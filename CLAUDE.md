...
- **封面默认板式 = 标题(两行) + 一句话核心句 + 一张论文真实原图,三样,没有别的**(2026-08-05 定板式,2026-08-10 改配图):**来源/溯源信息移到目录卡**,不再放封面(封面换点击、目录做信任判断+导航)。**封面图必须是从当期引用论文里真实截出来的图,自制 SVG 概念图已停用**(2026-08-10 定;背景介绍期同样适用,从当期引用的任一篇里选)。裁图三条:与标题同向、裁切完整、去掉面板字母。经 `cover_card(..., concept_image=..., title_size=...)` 传入(参数名沿用,内容含义已改)。**标题字号可用 `title_size` 调大到标题块约占封面 1/4**(内容宽 880px:120px 字号下每行最多 7 字);**`title_top` 可收紧标题上方的默认 200px 留白,把版面让给图**(2026-08-06 加,默认不变)。完整规则见 SKILL.md Step 7 §封面 + 目录卡。
